...
export function SoldRangeBar({
  ...
  const midPct = range ? rangePercent(range.mid, range.low, range.high) : 50;
  const marketPct =
    range == null || market == null ? undefined : rangePercent(market, range.low, range.high);
  const thin = range ? isThinSoldCount(range.soldCount) : false;
  ...
  return (
    <div className="range">
      {range ? (
        ...
        <p className="range-caption">Solds unavailable</p>
      ) : null}
